...
class MarketSumCodesSpider(scrapy.Spider):
    name = "market_sum_codes"
    custom_settings = {
    ...
    def __init__(self, out: str | None = None, *args, **kwargs):
        """
        -a out=파일명 으로 출력 경로 지정 가능. 기본: codes_all.txt
        """
        super().__init__(*args, **kwargs)
        self.out = out or "codes_all.txt"
        self._codes = set()

    def start_requests(self):
        # 코스피(0) + 코스닥(1) 1페이지부터 시작
        for sosok in (0, 1):
            url = f"{BASE}?sosok={sosok}&page=1"
            yield scrapy.Request(
                url,
                callback=self.parse_list,
                cb_kwargs={"sosok": sosok, "page": 1},
                dont_filter=True,
            )

    def parse_list(self, response, sosok: int, page: int):
        # 요청하신 위치: 2번째 컬럼의 <a>에서 code 파라미터 추출
        ...
        # 다음 페이지가 있으면 계속 크롤링
        if page < last_page:
            next_url = f"{BASE}?sosok={sosok}&page={page+1}"
            yield scrapy.Request(
                next_url,
                callback=self.parse_list,
                cb_kwargs={"sosok": sosok, "page": page + 1},
                dont_filter=True,
            )
    ...
    def closed(self, reason):
        ...
        with open(self.out, "w", encoding="utf-8") as f:
            f.write("\n".join(codes_sorted))
        self.logger.info("Saved %d codes to %s", len(codes_sorted), self.out)
